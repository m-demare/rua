...
use syn::{parse_macro_input, ItemFn, PatType, FnArg, TypePath, Type, TypeReference};
use quote::quote;
...
fn wrap_output(input_fn: &ItemFn) -> proc_macro2::TokenStream {
    ...
    match wrap_output_with {
        ...
        OutputWrappers::IntoResult => quote!(
            match res {
                Ok(r) => Ok(r.into()),
                Err(e) => Err(e),
            }
...
fn get_parameters(input_fn: &ItemFn, args: &MacroArgs) -> (Vec<proc_macro2::TokenStream>, proc_macro2::TokenStream) {
    let mut inputs = input_fn.sig.inputs.iter().peekable();
    let mut cant_params = inputs.len();
    let mut params = match inputs.peek() {
        Some(FnArg::Typed(PatType{ty: box Type::Reference(TypeReference { elem: box Type::Path(TypePath { path, .. }), .. }), ..})) 
