...
impl Expression {
    ...
    pub fn eval(&self, env: Rc<RefCell<Scope>>) -> RuaResult {
        ...
        match self {
            ...
            Self::Plus(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()? + e2.eval(env)?.into_number()?)),
            Self::Minus(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()? - e2.eval(env)?.into_number()?)),
            Self::Times(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()? * e2.eval(env)?.into_number()?)),
            Self::Div(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()? / e2.eval(env)?.into_number()?)),
            Self::Mod(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()? % e2.eval(env)?.into_number()?)),
            Self::Exp(box (e1, e2)) => Ok(V::Number(e1.eval(env.clone())?.into_number()?.powf(e2.eval(env)?.into_number()?))),
            Self::Eq(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())? == e2.eval(env)?)),
            Self::Neq(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())? != e2.eval(env)?)),
            Self::Le(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())?.into_number()? <= e2.eval(env)?.into_number()?)),
            Self::Ge(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())?.into_number()? >= e2.eval(env)?.into_number()?)),
            Self::Lt(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())?.into_number()? < e2.eval(env)?.into_number()?)),
            Self::Gt(box (e1, e2)) => Ok(V::Bool(e1.eval(env.clone())?.into_number()? > e2.eval(env)?.into_number()?)),
            Self::And(box (e1, e2)) => e1.and(e2, env),
            Self::Or(box (e1, e2)) => e1.or(e2, env),
            Self::Dotdot(box (e1, e2)) => Ok(V::String((e1.eval(env.clone())?.into_str()?.to_string() + &*e2.eval(env)?.into_str()?).into())),
