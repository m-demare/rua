...
fn test_interpret_aux<F: FnOnce(&mut Vm) -> Result<RuaVal, EvalErrorTraced>>(input: &str, output: F){
    let mut vm = Vm::new();
...
test_interpret!(string_concat, "return 'hello' .. ' ' .. 'world'", |vm| Ok(vm.new_string("hello world".into()).into()));
...
test_interpret!(r#while, "local i = 0
    while i < 42 do
        i = i + 1
    end
    return i", |_| Ok(42.0.into()));

test_interpret!(function, "
local function foo()
    return 1337
end
return foo()", |_| Ok(1337.0.into()));
test_interpret!(function_shadows_outer_var, "
local n = 5
function add(n, m)
    return n + m
end
...
test_interpret!(recursion, "
local function fact(n)
    if n < 2 then return 1 end
    return n * fact(n-1)
end
return fact(5)", |_| Ok(120.0.into()));
test_interpret!(recursion2, "
...
test_interpret!(stack_trace2, "function foo()
...
test_interpret!(native_non_native_nested_call, "
function foo()
    local function bar()
        local function baz()
            return 7
...
test_interpret!(table_index_num1, "
    local a = {1, b = true, 2, [2+3] = false}
    return a[1]",
    |_| Ok(1.0.into()));
test_interpret!(table_index_num2, "
    local a = {1, b = true, 3, [2+3] = false}
    return a[2]",
    |_| Ok(3.0.into()));
test_interpret!(table_index_char, "
    local a = {1, b = true, 2, [2+3] = false}
    return a['b']",
    |_| Ok(true.into()));
test_interpret!(table_index_num3, "
    local a = {1, b = true, 2, [2+3] = false}
    return a[5]",
    |_| Ok(false.into()));
test_interpret!(table_index_bool, "
    local a = {1, b = true, 2, [2+3] = false}
    return a[false]",
    |_| Ok(RuaVal::Nil));

test_interpret!(field_access1, "
    local a = {1, b = true, 2, [2+3] = false}
    return a.b",
    |_| Ok(true.into()));
test_interpret!(field_access2, "
    local a = {1, b = true, 2, [2+3] = false}
    return a.c",
    |_| Ok(RuaVal::Nil));
...
test_interpret!(call_with_more_args, "
function foo(arg)
    return arg
end
return foo(1, 2)", |_| Ok(1.0.into()));

test_interpret!(closure_read_local, "
local val = 1337
local function foo()
    return val
end
return foo()", |_| Ok(1337.0.into()));
test_interpret!(closure_write_local, "
local val = 1337
local function foo()
    val = 123
end
...
local f = foo(3)
return f(5) + f(2)", |_| Ok(16.0.into()));
test_interpret!(more_closures2, "
...
test_interpret!(many_closures_reference_same_local, "
local function foo()
    local val = 3
    local function bar()
        return val
...
test_interpret!(closure_that_outlive_variable_from_block, "
local function foo()
    local func
    if true then
        local val = 1337
...
test_interpret!(closure_that_outlive_variable_from_parent_function, "
local function foo()
    local val = 1337
    local function bar()
        return val
...
test_interpret!(closures_share_closed_over_variable, "
local function foo()
    local val = 1
...
test_interpret!(table_field_assignment1, "
    local a = {}
    a[3 + 2] = 8
    return a[5]
", |_| Ok(8.0.into()));

test_interpret!(table_field_assignment2, "
    local a = {}
    a.b = 9
    return a['b']
", |_| Ok(9.0.into()));
